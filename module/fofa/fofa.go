...
func GetConfig(fofaIni string) Config {
...
func fofa_api(keyword string, email string, key string, page int, size int) string {
	...
	api_request := fmt.Sprintf("https://fofa.info/api/v1/search/all?email=%s&page=%d&size=%d&key=%s&qbase64=%s&fields=protocol,host,ip,port", strings.Trim(email, " "), page, size, strings.Trim(key, " "), encodeString)
	return api_request
}
...
func FofaSearch(query string, size int, page int, fofaIni string) []string {
	//color.RGBStyleFromString("244,211,49").Println("请耐心等待fofa搜索......")
	fofa := GetConfig(fofaIni)
	keyword := query
	url := fofa_api(keyword, fofa.Email, fofa.Fofa_token, page, size)
	res := fofahttp(url, fofa.Fofa_timeout)
	color.RGBStyleFromString("244,211,49").Println("一共有" + strconv.Itoa(res.Size) + "个结果")
	var targets []string
	targets = make([]string, 0)
	for _, result := range res.Results {
		u := result[0] + "://" + result[1]
		u = strings.Replace(u, "http://http://", "http://", 1)
		u = strings.Replace(u, "https://https://", "https://", 1)
